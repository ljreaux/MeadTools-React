...
import StoreRecipe from "./StoreRecipe.jsx";
import { getRecipeById, setLocalStorage } from "../Utils/API";
...
function Home({ token }) {
  ...
  useEffect(() => {
    if (recipeId) {
      getRecipeById(recipeId, token).then((res) => {
        setLocalStorage(res.recipe);
      });
    }
  }, [recipeId]);
  ...
  useEffect(() => {
    ...
    setRecipe({
      ...
      volUnits,
      rowCount,
      submitted: displayMainResults,
      yeastInfo: mainCalcYeastInfo,
      yeastBrand: mainCalcYeastBrand,
      nuteSchedule: preferredSchedule,
      numOfAdditions: mainCalcNuteInfo.addNum,
      extraIngredients,
    });
  }, [
    storedInput,
  ...
  return (
    <>
      ...
      <div
        className={`max-h-screen flex items-center flex-col font-serif md:text-2xl lg:text-3xl text-textColor text-sm ${
          loading && "hidden"
        }`}
      >
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <div className="flex flex-col justify-center">
            ...
            />
            <StoreRecipe token={token} recipe={recipe} setRecipe={setRecipe} />
